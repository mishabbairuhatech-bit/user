...
const PaymentsPage = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: [QUERY_KEY.PAYMENT_RECEIPTS_LIST, 'payment', page, pageSize, debouncedSearch, sortColumn, sortDirection],
    queryFn: async () => {
      const params = {
        type: 'payment',
        page,
  ...
  const payments = data?.data?.items || data?.items || [];
  ...
  const handleRowClick = (row) => {
    if (isDesktop) {
      ...
      setSelectedPaymentId(selectedPaymentId === row.id ? null : row.id);
    } else {
      navigate(`/admin/banking/payments/${row.id}`);
    }
  };
  ...
  const handleAddPayment = () => {
    ...
    if (isDesktop) {
      ...
      navigate('/admin/banking/payments/create');
    }
  };
  ...
  return (
    <div className="flex flex-col gap-4 h-[calc(100vh-80px)] lg:h-[calc(100vh-96px)]">
      <PageHeader
        ...
        breadcrumb={{ items: [{ label: 'Banking' }, { label: 'Payments' }] }}
        sticky
      >
        {canCreate && (
          <Button size="sm" prefixIcon={Plus} onClick={handleAddPayment}>New Payment</Button>
        )}
      </PageHeader>
          ...
          rightPanel={
            (showCreateForm && isDesktop && canCreate) ? (
              <div className="h-full bg-white dark:bg-[#121212] rounded-2xl border border-gray-100 dark:border-[#424242] shadow-sm overflow-hidden">
                <PaymentCreateForm onClose={() => setShowCreateForm(false)} />
              </div>
            ) : (!showCreateForm && selectedPaymentId && isDesktop) ? (
              <div className="h-full bg-white dark:bg-[#121212] rounded-2xl border border-gray-100 dark:border-[#424242] shadow-sm overflow-hidden">
                <PaymentDetailPanel
