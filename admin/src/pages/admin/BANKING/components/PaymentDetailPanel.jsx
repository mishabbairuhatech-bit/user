...
const PaymentDetailPanel = ({ paymentId, onClose }) => {
  const navigate = useNavigate();
  const { data, isLoading } = useQuery({
    queryKey: [QUERY_KEY.PAYMENT_RECEIPT_DETAIL, paymentId],
    queryFn: async () => {
      const res = await api.get(`${API.PAYMENT_RECEIPTS}/${paymentId}`);
      return res.data;
    },
    enabled: !!paymentId,
  });
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 dark:border-[#424242] flex-shrink-0">
        ...
        <div className="flex items-center gap-1">
          <button
            onClick={() => navigate(`/admin/banking/payments/${paymentId}`)}
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide px-5 py-4">
        <div className="space-y-3.5">
          <Row label="Voucher #" value={
            <span className="font-mono">{payment.voucher_number || '-'}</span>
          } />
