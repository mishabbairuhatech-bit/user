...
const POSPage = () => {
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-100 dark:bg-[#0a0a0a]">
      {/* Header */}
      <POSHeader
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {/* Products grid */}
          <div className={`flex-1 overflow-y-auto bg-gray-50 dark:bg-[#0a0a0a] scrollbar-hide border-2 transition-colors ${
      ...
      {/* Search indicator - full width bar */}
      {(isSearchMode || searchQuery) && (
        <div className="fixed top-10 left-0 right-0 z-50 px-4 py-2 bg-white dark:bg-[#1a1a1a] border-b border-primary-400 dark:border-primary-600 shadow-lg flex items-center gap-3">
          <span className="text-sm font-medium text-primary-600 dark:text-primary-400">Search:</span>
          <div className="flex-1 text-sm font-medium text-gray-900 dark:text-white">
            {searchQuery || <span className="text-gray-400 animate-pulse">type to search...</span>}
          </div>
          <span className="text-xs text-gray-400">Press Esc to cancel</span>
          <button
            onClick={() => { setSearchQuery(''); setIsSearchMode(false); }}
            className="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-[#2a2a2a] rounded-full"
          >
            ×
          </button>
        </div>
